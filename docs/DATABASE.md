...
WorkTime uses **PostgreSQL 16** with **Prisma 5** as the ORM. Schema lives at `packages/database/prisma/schema.prisma` — shared across backend, seed scripts, and any workers that need typed database access.
...
B2C solo-freelancer workspace. Not tied to a Company — freelancers use WorkTime without creating a business.
